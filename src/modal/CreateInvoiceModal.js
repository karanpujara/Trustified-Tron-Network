...
function CreateInvoiceModal(props) {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { resetForm }) => {
      try {
        ...
        const randNo = await getRandom(1,1000);;
        setLoading(true);
        const formData = {
          invoiceNumber:randNo,
  ...
  async function storage(files, formData) {
    ...
    // );
    invoice.set("username", userAdd && userAdd);
    invoice.set("ethAddress", userAdd && userAdd);
    invoice.set(
      "avatar",
